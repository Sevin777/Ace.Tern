 ...
 *      Tern:  0.6.3
 *
 * 10.4.2014- Updated RequireJS plugin to new version
 */
...
(function(root, mod) {
    ...
    function skipBlockComment() {
        ...
        if (options.onComment) options.onComment(true, input.slice(start + 2, end), start, tokPos,
    ...
    function skipLineComment() {
        ...
        if (options.onComment) options.onComment(false, input.slice(start + 2, tokPos), start, tokPos,
        startLoc, options.locations && new Position);
    }
    ...
    function parseStatement() {
        ...
        default:
            ...
            if (starttype === _name && expr.type === "Identifier" && eat(_colon)) return parseLabeledStatement(node, maybeName, expr);
...
(function(root, mod) {
    ...
    function lookAhead(n) {
        ...
        if (!ahead.length) token = copyToken(token);
    ...
    function SourceLocation(start) {
        this.start = start || token.startLoc || {
            line: 1,
            column: 0
        };
        this.end = null;
    ...
    function startNodeFrom(other) {
        ...
        if (options.locations) node.loc = new SourceLocation(other.loc.start);
        return node;
    }
    ...
    function parseStatement() {
        ...
        case tt._for:
            ...
            if (token.type === tt._var) {
                ...
                next();
                parseVar(init, true);
                if (init.declarations.length === 1 && eat(tt._in)) return parseForIn(node, init);
                return parseFor(node, init);
            }
            var init = parseExpression(false, true);
            if (eat(tt._in)) {
    ...
    function parseExprOp(left, minPrec, noIn, indent, line) {
        ...
        if (prec != null && (!noIn || token.type !== tt._in)) {
            if (prec > minPrec) {
                ...
                node.operator = token.value;
                next();
                if (curLineStart != line && curIndent < indent && tokenStartsLine()) node.right = dummyIdent();
                else node.right = parseExprOp(parseMaybeUnary(noIn), prec, noIn, indent, line);
    ...
    function parseSubscripts(base, noCalls, startIndent, line) {
        for (;;) {
            if (curLineStart != line && curIndent <= startIndent && tokenStartsLine()) {
                if (token.type == tt.dot && curIndent == startIndent)--startIndent;
                else return base;
            }
    ...
    function parseObj() {
        ...
        pushCx();
        next();
        var propIndent = curIndent,
            line = curLineStart;
        while (!closes(tt.braceR, propIndent, line)) {
            var name = parsePropertyName();
            if (!name) {
                if (isDummy(parseExpression(true))) next();
                eat(tt.comma);
                continue;
            }
            var prop = {
...
(function(mod) {
    ...
    function resolveFile(srv, localFiles, name) {
        ...
        infer.withContext(srv.cx, function() {
            ...
            if (m = text.match(/(?:"([^"]*)"|([\w$]+))\s*:\s*function\b/)) {
                ...
                if (objNode && objNode.node.objType) var inObject = {
    ...
    function findTypeAt(_srv, query, file) {
        ...
        if (expr.node.type == "Identifier") var exprName = expr.node.name;
        else if (expr.node.type == "MemberExpression" && !expr.node.computed) var exprName = expr.node.property.name;
...
(function(mod) {
        ...
        walk.simple(ast, {
            ...
            ObjectExpression: function(node, scope) {
                for (var i = 0; i < node.properties.length; ++i) {
                    var prop = node.properties[i],
                        key = prop.key;
                    if (key.commentsBefore) interpretComments(prop, key.commentsBefore, scope,
    ...
    function jsdocInterpretComments(node, scope, aval, comments) {
        ...
        for (var i = 0; i < comments.length; ++i) {
            ...
            var decl = /(?:\n|$|\*)\s*@(type|param|arg(?:ument)?|returns?)\s+(.*)/g,
                m;
