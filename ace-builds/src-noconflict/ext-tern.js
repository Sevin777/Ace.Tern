        ...
        this.updateCompletions = function(keepPopupPosition) {
            ...
            this.gatherCompletions(this.editor, function(err, results) {
                ...
                if (!matches || !matches.length)
                    return detachIfFinished(matches);
                ...
                if (!filtered.length)
                    return detachIfFinished(filtered);
                if (filtered.length == 1 && filtered[0].value == prefix && !filtered[0].snippet) {
                    // this.popup.setData(filtered);
                    return detachIfFinished(filtered);
                }
...
ace.define("ace/tern/tern_server",["require","exports","module","ace/range","ace/lib/dom"], function (require, exports, module) {
    ...
    var aceCommands = {
        ternJumpToDef: {
            name: "ternJumpToDef",
            exec: function (editor) {
                editor.getSession().ternServer.jumpToDef(editor);
            },
            bindKey: "Alt-."
        },
        ternJumpBack: {
            name: "ternJumpBack",
            exec: function (editor) {
                editor.getSession().ternServer.jumpBack(editor);
            },
            bindKey: "Alt-,"
        },
        ternShowType: {
            name: "ternShowType",
            exec: function (editor) {
                editor.getSession().ternServer.showType(editor);
            },
            bindKey: "Ctrl-I"
        },
        ternFindRefs: {
            name: "ternFindRefs",
            exec: function (editor) {
                editor.getSession().ternServer.findRefs(editor);
            },
            bindKey: "Ctrl-E"
        },
        ternRename: {
            name: "ternRename",
            exec: function (editor) {
                editor.getSession().ternServer.rename(editor);
            },
        ...
        ternRefresh: {
            ...
            exec: function (editor) {
                var full = false;
                if (editor.getSession().ternServer.refreshDocLastCalled != null) {
                    if (new Date().getTime() - editor.getSession().ternServer.refreshDocLastCalled < 1000) { //less than 1 second
                        full = true;
                    }
                }
                editor.getSession().ternServer.refreshDocLastCalled = new Date().getTime();
                editor.getSession().ternServer.refreshDoc(editor, full);
            },
            bindKey: "Alt-R"
        },
    };
    ...
    TernServer.prototype = {
        ...
        closeArgHints: function (editor) {
            closeArgHints(this, editor);
        },
        ...
        ts.request(editor, {
            ...
            function (error, data) {
                ...
                if (!bindPopupSelect()) {
                    ...
                    setTimeout(popupSelectionChanged, 10); //call once if popupselect bound exited to show tooltip for first item
                }
                function bindPopupSelect() {
                    ...
                    setTimeout(popupSelectionChanged, 10); //fire once after first bind
    ...
    function loadExplicitVsRefs(ts, editor) {
        if (!editor.getSession().ternServer || !editor.getSession().ternServer.enabledAtCurrentLocation(editor)) {
            return;
        }
                    ...
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState == 4) {
                            if (xhr.status == 200) {
                                ...
                                editor.getSession().ternServer.addDoc(path, xhr.responseText);
                            }
                            else {
...
ace.define("ace/tern/tern",["require","exports","module","ace/config","ace/snippets","ace/autocomplete/text_completer","ace/autocomplete","ace/tern/tern_server","ace/editor"], function (require, exports, module) {
    ...
    Autocomplete.startCommand = {
        ...
        exec: function (editor, e) {
            ...
            if (editor.getSession().ternServer && editor.$enableTern) {
                if (editor.getSession().ternServer.enabledAtCurrentLocation(editor)) {
                    editor.completers.push(editor.getSession().ternServer);
                    editor.getSession().ternServer.aceTextCompletor = textCompleter; //9.30.2014- give tern the text completor
                }
                else {
    ...
    var onAfterExec_Tern = function (e, commandManager) {
        if (/*e.command.name === 'backspace' || */(e.command.name === "insertstring" && (e.args === "." || /^[a-zA-Z_0-9\$]$/.test(e.args)))) {
            console.log('afterExec');
            if (e.editor.getSession().ternServer && e.editor.getSession().ternServer.enabledAtCurrentLocation(e.editor)) {
                ...
                if (tok) {
                    if (tok.type !== 'string' && tok.type.toString().indexOf('comment') === -1) {
                        try {
                            e.editor.getSession().ternServer.lastAutoCompleteFireTime = null; //reset since this was not triggered by user firing command but triggered automatically
                        }
                        catch (ex) {}
        ...
        enableTern: {
            set: function (val) {
                ...
                if (val) {
                    ...
                    delete self.getSession().ternServer;
